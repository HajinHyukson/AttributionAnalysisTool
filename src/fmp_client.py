...
class FMPClient:
    ...
    def get_company_profile(self, ticker: str) -> dict[str, Any]:
        ...
        return self._first_record(payload, label=f"profile for {symbol}")

    def _first_record(self, payload: Any, *, label: str) -> dict[str, Any]:
        if isinstance(payload, list):
            if not payload:
                raise NotFoundError(f"No {label} data returned for request (check symbol/plan).")
            first = payload[0]
            if not isinstance(first, dict):
                raise APIError(f"Malformed {label} response: expected object records.")
            return first
        if isinstance(payload, dict):
            return payload
        raise APIError(f"Malformed {label} response: expected object or object list.")
